...
- A real Google Gemini API key (free, no credit card - aistudio.google.com)
...
src/
  ...
  server/                  # The real backend
    ...
    config/db.js
    models/                # User (auth only), Enrollment (one per course), ContentModule, SectionAssessment
    routes/                # auth, content, enrollments
...
route logic run without live MongoDB/Gemini access in an offline
...
#   GEMINI_API_KEY     - a real key from aistudio.google.com (free, no card)
npm start
```
